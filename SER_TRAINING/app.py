...
import pickle
import os, glob, pickle,subprocess
...
def convert_to_wav():
    ...
    try:
        ...
        audio_path = 'myaudio/' + audio_file.filename
        audio_file.save(audio_path)

        # Convert the audio file to WAV using ffmpeg
        wav_path = 'myaudio/' + os.path.splitext(audio_file.filename)[0] + '.wav'
        command = ['ffmpeg', '-i', audio_path, '-acodec', 'pcm_s16le', '-ar', '44100', wav_path]
        subprocess.run(command, check=True)

        # Return the path to the converted WAV file
        return jsonify({'wav_path': wav_path}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
